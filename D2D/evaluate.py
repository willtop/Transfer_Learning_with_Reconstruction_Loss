...
N_REALIZATIONS = 1000
BENCHMARKS = ["Full Power"]

if(__name__ =='__main__'):
    h_est = np.load("Data_D2D/h_est_test_{}.npy".format(SETTING_STRING))
    assert np.shape(h_est)[0] == N_TEST
    ...
    if "Geometric Programming" in BENCHMARKS:
        power_controls["Geometric Programming"] = GP_power_control(h_est)
    ...
    _, pc = regular_net(torch.tensor(h_est, dtype=torch.float32).to(DEVICE))
    ...
    _, pc = robust_net(torch.tensor(h_est, dtype=torch.float32).to(DEVICE))
    ...
    for method_key in power_controls.keys():
        sinrs = compute_SINRs(power_controls[method_key], h_est)
    ...
    csi = add_fadings(np.tile(h_est, reps=[N_REALIZATIONS, 1, 1]), channel_type='CL')
    for method_key in power_controls.keys():
