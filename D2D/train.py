...
if(__name__=="__main__"):
    ...
    Sum-Rate Training
    """
    g_sumRate, fp = np.load("Data/g_sumRate_{}.npy".format(SETTING_STRING)), np.load("Data/fp_{}.npy".format(SETTING_STRING))
    ...
    Min-Rate Training
    """
    g_minRate, fp = np.load("Data/g_sumRate_{}.npy".format(SETTING_STRING)), np.load("Data/fp_{}.npy".format(SETTING_STRING))
    g_sumRate_train, g_sumRate_valid = g_sumRate[:N_SAMPLES['SumRate']['TRAIN']], g_sumRate[-N_SAMPLES['SumRate']['VALID']:]
    fp_train, fp_valid = fp[:N_SAMPLES['SumRate']['TRAIN']], fp[-N_SAMPLES['SumRate']['VALID']:]
    n_train = np.shape(g_sumRate_train)[0]
    assert n_train % MINIBATCH_SIZE == 0
